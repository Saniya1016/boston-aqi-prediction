...
class LightGBMAQI:
    ...
    def short_description(self):
        return "LightGBM Regressor using tuned hyperparameters (leaves=31, n_est=900, lr=0.01)."

    def _build_features(self, df):
        # Replicating notebook feature engineering strictly
    ...
    def plot_results(self, data_dict, fig):
        ...
        # Scatter
        ax1.scatter(self.test_data['actual_AQI'], self.test_data['predicted_AQI'], alpha=0.5, color='green')
        lims = [0, max(self.test_data['actual_AQI'].max(), self.test_data['predicted_AQI'].max())]
        ax1.plot(lims, lims, 'r--')
        ax1.set_title(f"LightGBM Performance (R²={self.metrics['R2']:.2f})")
        ax1.set_xlabel("Actual")
        ax1.set_ylabel("Predicted")
        
        # Timeline
        ax2.plot(self.test_data['date'], self.test_data['actual_AQI'], label="Actual", color='gray', alpha=0.5)
        ax2.plot(self.test_data['date'], self.test_data['predicted_AQI'], label="LightGBM", color='green', alpha=0.8)
