...
# Added: --ExecutePreprocessor.kernel_name=$(VENV)
.PHONY: run-aqi-nb
run-aqi-nb: install download-data
	@echo "Running AQI Prediction Notebook..."
	$(JUPYTER) nbconvert --to notebook --execute --inplace --ExecutePreprocessor.kernel_name=$(VENV) $(AQI_NOTEBOOK)
...
run-pollen-nb: install download-data
	...
	$(JUPYTER) nbconvert --to notebook --execute --inplace --ExecutePreprocessor.kernel_name=$(VENV) $(POLLEN_NOTEBOOK)
